...
# ])
dino_preprocess = transforms.Compose([  # 新的参数试试--这个不到1万张测试时是74.18%，比上面那个好
...
for dirpath, _, filenames in os.walk(test_image_directory):
    for test_filename in filenames:
        if test_filename.endswith(('.png', '.jpg', '.JPEG')):
            ...
            similarity_with_text = (1.0 * image_features @ text_features.T).softmax(dim=-1)  # 200
            ...
            # max_similarity_text, _ = similarity_with_text.topk(1)
            # max_similarity_clip, _ = similarity_with_clip.topk(1)
            # max_similarity_dino, _ = similarity_with_dino.topk(1)
            # zx_text.append(max_similarity_text.item())
            # zx_clip.append(max_similarity_clip.item())
            # zx_dino.append(max_similarity_dino.item())
            # #             看看过程！！全！索引2+标签3+分数1
            #            # # 获取每个模型最高相似度值1及对应的索引2
            # 获取每个模型最高相似度值及对应的索引
            # max_similarity_text, top_indices_text = similarity_with_text.topk(1)
            # max_similarity_clip, top_indices_clip = similarity_with_clip.topk(1)
            # max_similarity_dino, top_indices_dino = similarity_with_dino.topk(1)
            # # 打印每个模型的最高相似度值对应的索引
            # print(f"Top Index with Text: {top_indices_text.item()}") #
            # print(f"Top Index with CLIP: {top_indices_clip.item()}") #
            # print(f"Top Index with DINO: {top_indices_dino.item()}") #
            # # 根据索引获取对应的类别标签
            # top_label_text = your_labels_list[top_indices_text.item()]
            # top_label_clip = reference_filenames_clip[top_indices_clip.item()]
            # top_label_dino = reference_filenames[top_indices_dino.item()]
            # # 打印预测到的第一名类别标签及其对应的相似度值
            # print("Top Predicted Label with Text:")
            # print(f"Label: {top_label_text}, Similarity: {max_similarity_text.item()}")
            # print("\nTop Predicted Label with CLIP:")
            # print(f"Label: {top_label_clip}, Similarity: {max_similarity_clip.item()}")
            # print("\nTop Predicted Label with DINO:")
            # print(f"Label: {top_label_dino}, Similarity: {max_similarity_dino.item()}")
            # . 在这里添加总相似度的计算（按照类别标签相同的进行相加），并输出总相似度前5名的类别标签和相似度的值
            # 初始化一个与 your_labels_list 等长的零向量，用于累计每个类别的总相似度
            # total_similarities = torch.zeros(len(your_labels_list), device=device)#200
            # total_similarities += max_similarity_text.item() * similarity_with_text.squeeze()#200
            ...
            #     total_similarities[text_index] += max_similarity_clip.item() * similarity_with_clip[:, i].squeeze()#400->200#i！！！！！
            #     total_similarities[text_index] += max_similarity_dino.item() * similarity_with_dino[:, i].squeeze()#少写了这一行怎么回事
            ...
            total_dino_similarities = torch.softmax(total_dino_similarities * 1.0, dim=-1)  # softmax
            ##获取每个模型最高相似度值 #zhixin1独有   #打印每个图的3个置信度
