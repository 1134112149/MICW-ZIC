...
# #方法一
dino_preprocess = transforms.Compose([  # ？72.17%
...
# 方法二
# dino_preprocess = transforms.Compose([#这俩都行这个多一张正确的？！#72.17%
...
def process_and_predict(image, clip_model, dino_model, clip_preprocess, dino_preprocess):
    ...
    # cos_similarity = F.cosine_similarity(clip_image_features, text_features, dim=-1)#余弦相似度
    # similarity_with_text = F.softmax(cos_similarity, dim=-1)
    similarity_with_text = (1.0 * clip_image_features @ text_features.T).softmax(dim=-1)  ####没加余弦相似度
    ...
    total_dino_similarities = torch.softmax(total_dino_similarities * 40.0, dim=-1)  # softmax
    # 计算另外两个的熵entropy
