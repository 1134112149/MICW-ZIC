...
def tinyimage_semantic_spit_generator():#生成Tiny ImageNet数据集的语义划分
    tinyimage_splits = splits_2020["tiny_imagenet"]
    #print("tinyimage_splits:", tinyimage_splits)  ###
    dataset = ImageFolder(root='./data/tiny-imagenet-200/val')
    a = dataset.class_to_idx
    reverse_a = {v:k for k,v in a.items()}
    semantic_splits = [[],[],[],[],[]]
    for i, split in enumerate(tinyimage_splits):###
       print("i:", i)  ###
       #print("spliy:", split)  ###就是split.py里面的tiny_imagenet
       wnid_split = []
       for idx in split:
           wnid_split.append(reverse_a[idx])
       all = list(dataset.class_to_idx.keys())###
       print("all:", all)  ###val中的文件夹名字共200个（n014443537），每次每轮都一样
       seen = wnid_split
       print("seen:", seen)  ###根据split对应的idx

       #unseen = list(set(all)-set(seen))#原
       unseen = sorted(list(set(all) - set(seen)))#这样就有顺序了！
       print("unseen:", unseen)  ###每次运行都随机？？？因为集合是无序的。。但是每轮的都差不多
       seen.extend(unseen)
       #print("seen:", seen)  ###seen+unseen
       f = open('./dataloaders/imagenet_id_to_label.txt', 'r')
       imagenet_id_idx_semantic = f.readlines()
    ...
    print("semantic_splits:", semantic_splits)  ###
...
def tinyimage_single_isolated_class_loader(train=False):
    ...
    loaders_dict = {}
    if not train:#是这个，开头参数设定的
        for semantic_label in mappings_dict.keys():
            dataset = tinyimage_isolated_class(semantic_label, mappings_dict)###dataset?: <dataloaders.ZO_Clip_loaders.tinyimage_isolated_class object at 0x7fbef24f2130>
