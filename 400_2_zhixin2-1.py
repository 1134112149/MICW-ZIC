...
dino_preprocess = transforms.Compose([  # 131时top1是73.52%
    transforms.Resize(256),
    transforms.CenterCrop(224),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
])
# dino_preprocess = transforms.Compose([   #？这个更好了。。。。算了没时间做了
...
for dirpath, _, filenames in os.walk(test_image_directory):
    for test_filename in filenames:
        if test_filename.endswith(('.png', '.jpg', '.JPEG', '.webp')):
            ...
            total_similarities += 1 / (text_entropy.item() + 1e-6) * similarity_with_text.squeeze()  # 200
            ##相同类别分数相加的方式：前面没softmax!!!!!这里每个方法算完成400->200维，再softmx
            # clip
            ...
            total_clip_similarities = torch.softmax(total_clip_similarities * 3.0, dim=-1)  # softmax改这里温...定死4
            # dino
