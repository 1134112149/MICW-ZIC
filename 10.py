# Prediction accuracy: 85.01%
# AUROC: 0.9848738333333334
import os
import clip
import torch
...
# 方法一
dino_preprocess = transforms.Compose([  # 温度1230：92.96%
...
#     #方法二
# dino_preprocess = transforms.Compose([         #温度1350：91.96%   #温度1230：92.66%
...
# print("reference_dino_features shape:", reference_dino_features.shape)#30，768
# print("reference_dino_features transpose shape:", reference_dino_features.t().shape)#768，30
...
targets = []
probs_sum = []
indices = [8, 6, 1, 9, 0, 7]  # 0.99678
known_categories = [your_labels_list[i] for i in indices]
# known_categories = [0, 6, 4, 9, 1, 7]
...
def process_and_predict(image, clip_model, dino_model, clip_preprocess, dino_preprocess):
    ...
    similarity_with_text = (1.0 * clip_image_features @ text_features.T).softmax(dim=-1)  # 呜呜呜忘了用余弦相似度了
    ...
    similarity_with_dino = torch.mm(dino_features, reference_dino_features.t())  # ！！！不要再加unsqeeze了，已经二维了

    # #用余弦相似度处理
    # cos_similarity_1 = F.cosine_similarity(clip_image_features, text_features, dim=-1)
    # similarity_with_text = F.softmax(cos_similarity_1, dim=-1)
    # cos_similarity_2 = F.cosine_similarity(clip_image_features, reference_features_clip, dim=-1)
    # similarity_with_clip = F.softmax(cos_similarity_2.unsqueeze(0), dim=-1)
    # dino_image_input = dino_preprocess(image).unsqueeze(0).to(device)# 添加批量维度并转移到gpu
    # with torch.no_grad():
    #     # DINO的特征提取逻辑可能需要根据实际模型进行调整
    #     dino_output = dino_model(dino_image_input)
    #     dino_features = torch.nn.functional.normalize(dino_output, dim=1)
    # cos_similarity_3 = F.cosine_similarity(dino_features, reference_dino_features, dim=-1)
    # similarity_with_dino = F.softmax(cos_similarity_3.unsqueeze(0), dim=-1)
    ...
    # text
    total_similarities += (1 / (text_entropy.item() + 1e-6)) * similarity_with_text.squeeze()  # 200 text
    ...
    # zx_text.append(1/(text_entropy.item() + 1e-6)) ##怎么是tensor？
    # zx_clip.append(1/(clip_entropy.item() + 1e-6))
    # zx_dino.append(1/(dino_entropy.item() + 1e-6))
