...
# print("text_inputs.shape",text_inputs.shape)#[100,77]
text_features = clip_model.encode_text(text_inputs)
print("text_features.shape",text_features.shape)#[100,512]
#2.clip的                
...
print("reference_features_clip.shape",reference_features_clip.shape)#[200,512]
#3.dino的
...
print("reference_dino_features shape:", reference_dino_features.shape)#200，768
# print("reference_dino_features transpose shape:", reference_dino_features.t().shape)#768，200
...
def process_and_predict(image, clip_model, dino_model, clip_preprocess, dino_preprocess):
    ...
    # print("clip_image_input.shape",clip_image_input.shape)#[1,3,224,224]
    with torch.no_grad():
        clip_image_features = clip_model.encode_image(clip_image_input)
        # print("clip_image_features.shape",clip_image_features.shape)#[1,512]
    ...
    # print("similarity_with_text.shape",similarity_with_text.shape)#[1,100]
    # 计算测试图像的 CLIP 图像特征相似度
    similarity_with_clip = torch.mm(clip_image_features, reference_features_clip.transpose(0, 1))#400
    # print("similarity_with_clip.shape",similarity_with_clip.shape)#[1,200]
    # DINO特征提取
    dino_image_input = dino_preprocess(image).unsqueeze(0).to(device)# 添加批量维度并转移到gpu
    # print("dino_image_input.shape",dino_image_input.shape)#[1, 3, 224, 224]
    with torch.no_grad():
        ...
        # print("dino_features.shape",dino_features.shape)#[1, 768]
    similarity_with_dino = torch.mm(dino_features, reference_dino_features.t())#！！！
    # print("similarity_with_dino.shape",similarity_with_dino.shape)#[1, 200]
    #这是新的     2->1合并后才softmax  
...
# 相同类别分数相加的方式：前面没softmax!!!!!这里每个方法算完成200维，再softmx
    ...
    total_dino_similarities = torch.softmax(total_dino_similarities*50.0, dim=-1)#softmax#   ###cifar100这里得调，否则分数占比太小了
...
#在这里添加功能：输出这三个方法各自最高前五名的分数，以及对应的标签label
# After computing similarities with text, clip, and dino features:
    # top5_text_scores, top5_text_indices = similarity_with_text.topk(5)
    # top5_text_labels = [your_labels_list[i] for i in top5_text_indices[0].cpu().numpy()]
    # # After computing total_clip_similarities:
    # top5_clip_scores, top5_clip_indices = total_clip_similarities.topk(5)
    # top5_clip_labels = [your_labels_list[i] for i in top5_clip_indices.cpu().numpy()]
    # # After computing total_dino_similarities:
    # top5_dino_scores, top5_dino_indices = total_dino_similarities.topk(5)
    # top5_dino_labels = [your_labels_list[i] for i in top5_dino_indices.cpu().numpy()]
    # top5_text_scores = top5_text_scores.detach()
    # top5_clip_scores = top5_clip_scores.detach()
    # top5_dino_scores = top5_dino_scores.detach()
    # # To print each method's top five scores and labels:
    # print("Top 5 CLIP Text Scores and Labels:", list(zip(top5_text_scores.cpu().numpy(), top5_text_labels)))
    # print("Top 5 CLIP Image Scores and Labels:", list(zip(top5_clip_scores.cpu().numpy(), top5_clip_labels)))
    # print("Top 5 DINO Scores and Labels:", list(zip(top5_dino_scores.cpu().numpy(), top5_dino_labels)))
