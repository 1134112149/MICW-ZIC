...
# # # 初始化空列表来存储预测概率和真实标签###第一种auroc--不行
...
for dirpath, _, filenames in os.walk(test_image_directory):
    for test_filename in filenames:
        if test_filename.endswith(('.png', '.jpg', '.JPEG')):
            ...
            # total_similarities +=0.3*  similarity_with_text.squeeze()#200 text
            ...
            total_dino_similarities = torch.softmax(total_dino_similarities * 1.0, dim=-1)
            total_similarities += 0.3 * total_clip_similarities
            total_similarities += 0.4 * total_dino_similarities
